...
/*
 	...
 	구현하는 인터페이스 이다.
 	그리고, comparable에서는 compareTo()메서드를 재정의 해야하고,
 	comparator는 compare()메서드를 재정의 해야한다.
 	
 	-String클래스, Wrapper클래스,  Date클래스, File클래스에는
 	내부 정렬 기준이 구현되어 있다.(내부 정렬 기준은 보통 오름차순으로 처리되도록 구현되어 있다.)
...
public class ListSortTest {
	public static void main(String[] args) {
		...
		Collections.shuffle(list);//데이터를 섞어주는 역할을 함.(shuffle)
...
class Desc implements Comparator<String>{
	//compare()메서드를 이요해서 정렬하고자 하는 기준을 정한다.
	//compare()메서드의 반환값과 의미
	//반환값이  0은 두값이 같다.
	//반환값이 양수일 경우 ==>앞, 뒤의 순서를 바꾼다.
	//반환값이 음수 ==> 앞, 뒤의 순서를 바꾸지 않는다.
	
	//예) 오름차순일 경우 ==> 앞의 값이 크면 양수, 같으면 0, 앞의 값이 작으면 음수가 반환하도록 구현하면 된다.
