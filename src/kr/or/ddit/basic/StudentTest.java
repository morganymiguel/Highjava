...
public class StudentTest {
	/*
	 * 문제) 학번, 이름, 국어점수, 영어점수, 수학점수, 총점, 등수를 멤버로 갖는 Student클래스를 만든다. 이 Student클래스의
	 * 생성자에서는 학번, 이름, 국어점수, 영어점수, 수학점수만 매개변수로 받아서 초기화 처리를 한다.
	 * 
	 * 이 Student객체는 List에 저장하여 관리한다. List에 저장된 데이터들을 학번의 오름차순으로 정렬할 수 있는 내부 정렬 기준을
	 * 구현하고, 총점의 역순으로 정렬하는데 총점이 같으면 이름의 오름차순으로 정렬되는 외부 정렬 기준 클래스를 작성하여 정렬된 결과를
	 * 출력하시오. (단, 등수는 학생관리를 처리하는 클래스에서 구하도록 하는데 List에 전체 데이터가 추가된 후에 처리되도록 한다. 클래스명
	 * : StudentTest )
	 */
	// 등수를 구하는 메서드
	public void setRanking(List<Student> stdList) {
		for (Student std : stdList) { // 기준 데이터를 선택하는 반복문
			int rank = 1;// 처음에는 1들으로 설정해 놓고 시작한다.
	...
	public static void main(String[] args) {
		...
		stdList.add(new Student(1, "홍길동", 75, 95, 80));
		stdList.add(new Student(3, "성춘향", 90, 75, 70));
		stdList.add(new Student(7, "강감찬", 90, 95, 80));
		stdList.add(new Student(5, "변학도", 84, 90, 95));
